...
import {
  Check, Clock, AlertTriangle, HardHat,
  Receipt, Flag, MessageSquare, Calendar, DollarSign,
  Lock, ArrowUpRight, Upload, Shield, Ban, Banknote, SlidersHorizontal,
} from 'lucide-react';
...
/* ═══════ TAB: Overview ═══════ */
function OverviewTab({ item, user, role }) {
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <div className="text-lg font-semibold text-stone-900">{user}</div>
          <div className="text-xs text-stone-500">{role} · Field Owner</div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 text-sm text-stone-500 mb-1">
        ...
        <span>·</span>
        <span>Submitted {fmtDate(item.submittedDate)}</span>
      </div>

      <Divider />

      <SectionTitle>Reimbursement Details</SectionTitle>
      <FieldRow label="Spent On" value={item.spentOn} icon={HardHat} />
      <FieldRow label="Transaction Date" value={fmtDate(item.transactionDate)} icon={Calendar} />
      <FieldRow label="Submitted Date" value={fmtDate(item.submittedDate)} icon={Calendar} />
      ...
      {!item.receipt ? (
        ...
        <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4 flex items-center gap-3">
          ...
          <div className="flex-1">
            <div className="text-sm font-medium text-emerald-800">receipt_{item.id}.jpg</div>
            <div className="text-xs text-emerald-600">Uploaded {fmtShort(item.submittedDate)}</div>
          </div>
      ...
      <textarea
        ...
        placeholder="Add a job note — e.g., mileage for site inspection at Trust Hill..."
        rows={3}
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <div className="text-xs text-stone-500">
                {isWeekend ? 'Incurred on a weekend' : 'Weekday — no flag'}
              </div>
            </div>
          </div>
          {isWeekend && <span className="text-xs font-medium text-amber-700 bg-amber-100 px-2 py-0.5 rounded-full">Flagged</span>}
        </div>
...
function AccountingTab({ item }) {
  const isExportReady = item.receipt;
    ...
    <div>
      ...
      <div className={`flex items-center gap-2 px-3.5 py-2.5 rounded-lg border mb-4 ${
        ...
        {isExportReady
          ...
          : <><AlertTriangle size={14} className="text-amber-600" /><span className="text-xs font-medium text-amber-700">Not ready — missing receipt</span></>
        }
      </div>

      <FieldRow label="Spent On" value={item.spentOn} verified />
      <FieldRow label="Accounting Category" value="Reimbursement — Field Spend" verified />
      <FieldRow label="Amount" value={fmt(item.amount)} verified />
      ...
      <div className="space-y-2">
        {[
          { label: 'Receipt', ok: item.receipt, okText: 'Attached', failText: 'Missing' },
          ...
          { label: 'Approval status', ok: false, okText: 'Approved', failText: 'Pending review' },
        ].map((rule, i) => (
      ...
      <div className="text-xs text-stone-500 flex items-start gap-1.5 mt-4">
        ...
        Receipt + approval required before export.
      </div>
    </div>
  );
}
...
/* ═══════ TAB: Activity ═══════ */
function ActivityTab({ item, user }) {
  const activities = [
    { time: `${fmtShort(item.submittedDate)} · 9:15 AM`, who: user, action: 'Reimbursement submitted', icon: Receipt },
    ...(item.receipt ? [{ time: `${fmtShort(item.submittedDate)} · 9:16 AM`, who: user, action: 'Receipt uploaded', icon: Upload }] : []),
    { time: `${fmtShort(item.submittedDate)} · 9:17 AM`, who: 'System', action: 'Auto-assigned to project', icon: HardHat },
    { time: `${fmtShort(item.submittedDate)} · 9:18 AM`, who: 'System', action: 'Sent for manager review', icon: Clock },
    ...(!item.receipt ? [{ time: `${fmtShort(item.submittedDate)} · 9:20 AM`, who: 'System', action: 'Missing receipt — reminder sent', icon: Flag }] : []),
  ];
...
/* ═══════ MAIN: ReimbursementModal ═══════ */
const TABS = ['Overview', 'Job Context', 'Accounting', 'Activity'];

export default function ReimbursementModal({ open, onClose, onAction, item, user, role }) {
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={`Reimbursement — ${item.spentOn}`}
      subtitle={
      ...
      footer={
        <div className="flex items-center justify-center gap-3">
          {item.status === 'Pending review' && (
            <>
              <button
                onClick={() => onAction?.('pay', `Approved & paid reimbursement — ${user} (${fmt(item.amount)})`)}
              ...
              >
                <Banknote size={14} /> Approve & Pay
              </button>
              <button
                onClick={() => onAction?.('reject', `Rejected reimbursement — ${user} (${item.spentOn})`)}
              ...
              <button
                onClick={() => onAction?.('flag', `Changed spending limit for ${user}`)}
              ...
              >
                <SlidersHorizontal size={14} /> Change limit
              </button>
            </>
          )}
          {item.status !== 'Pending review' && (
    ...
    >
      ...
      {activeTab === 'Overview' && <OverviewTab item={item} user={user} role={role} />}
      {activeTab === 'Job Context' && <JobContextTab item={item} />}
      {activeTab === 'Accounting' && <AccountingTab item={item} />}
      {activeTab === 'Activity' && <ActivityTab item={item} user={user} />}
    </Modal>
  );
}
